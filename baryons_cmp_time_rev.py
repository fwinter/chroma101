...
# ==== Configuration ====
sink_type = "Shell_Point_Wilson_Baryons"
target_baryon = "17"
target_momentum = "1 1 0"
...
def func(input_file , row):
        tree = ET.parse(input_file)
        root = tree.getroot()
        baryons_path = f".//Wilson_hadron_measurements/elem/{sink_type}"
        baryons_elem = root.find(baryons_path)
        
        if baryons_elem is None:
            print(f"Error: Could not find path '{baryons_path}'")
            exit(1)
        
        found = False
        data = []
        
        for baryon in baryons_elem.findall("elem"):
            baryon_val = baryon.find("baryon_num")
            if baryon_val is not None and baryon_val.text.strip() == target_baryon:
                for mom in baryon.find("momenta").findall("elem"):
                    ...
                    if sink_mom is not None and sink_mom.text.strip() == target_momentum:
                        ...
                        print(f"  baryon_num  = {target_baryon}")
                        print(f"  sink_mom    = {target_momentum}\n")
                        for point in mom.find("barprop").findall("elem"):
...
func("hadspec.dat.xml",0)
func("hadspec_rev.dat.xml",2)
